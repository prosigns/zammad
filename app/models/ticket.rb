    ...
    Transaction.execute(local_options) do
      triggers.each do |trigger|
        ...
        if recursive == false && local_options[:loop_count] > 1
          message = "Do not execute recursive triggers per default until EchoDesk 2.0. With EchoDesk 2.0 and higher the following trigger is executed '#{trigger.name}' on Ticket:#{ticket.id}. Please review your current triggers and change them if needed."
          logger.info { message }
          return [true, message]
        end
