class App.Webhook extends App.Model
  ...
  @description = __('''
Webhooks make it easy to send information about events within EchoDesk to third-party systems via HTTP(S).

You can use webhooks in EchoDesk to send ticket, article, and attachment data whenever a trigger is performed. Just create and configure your webhook with an HTTP(S) endpoint and relevant security settings, then configure a trigger to perform it.
''')
